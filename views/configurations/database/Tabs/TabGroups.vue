    ...
    <div class="grid-1 grid-md-2 grid-lg-2 gap-15">
        <fieldset v-for="group in groups" :key="group.id">
            <legend>
                ...
                <button @click="generateSeeder(group)">
                    <i class="fas fa-seedling"></i>
                </button>
                <button @click="executeSeeder(group)">
            ...
            <form
                ...
                class="d-flex align-items-end mt-25"
            >
    ...
    httpRequest({
        ...
        .then((data) => {
            ...
            window.awesomeModal.alert(
                'Grupo añadido',
                'El grupo ha sido añadido correctamente'
            );
...
const addTable = (form) => {
    ...
    httpRequest({
        ...
        .then((data) => {
            ...
            window.awesomeModal.alert(
                'Tabla añadida',
                'La tabla ha sido añadida correctamente'
            );
...
const removeTable = (group_id, table) => {
    window.awesomeModal
        .confirm(
            '¿Está seguro?',
            '¿Está seguro que desea eliminar la tabla ' +
                table +
                ' del grupo ' +
                groups.find((group) => group.id == group_id).name +
                '?'
        )
        .then((result) => {
            if (result) {
                ...
                httpRequest({
                    url:
                        window.public_path + '/api/database/group/table/remove',
                    method: 'POST',
                    data: formData,
                })
                    .then((data) => {
...
const generateSeeder = (group) => {
    window.awesomeModal
        .confirm(
            'Generar seeder',
            '¿Está seguro de generar el seeder del grupo ' + group.name + '?'
        )
        .then((value) => {
            if (value) {
                window.awesomeModal.loading();
                ...
                httpRequest({
                    url:
                        window.public_path +
                        '/api/database/group/seeders/generate',
                    method: 'POST',
                    data: formData,
                })
                    .then((data) => {
                    ...
                    .catch((error) => {
                        ...
                        window.awesomeModal.alert(
                            'Error',
                            'Ha ocurrido un error al generar el seeder del grupo ' +
                                group.name
                        );
                    });
            }
...
const executeSeeder = (group) => {
    window.awesomeModal
        .confirm(
            'Ejecutar seeder',
            '¿Está seguro de ejecutar el seeder del grupo ' + group.name + '?'
        )
        .then((value) => {
            if (value) {
                window.awesomeModal.loading();
                ...
                httpRequest({
                    url:
                        window.public_path +
                        '/api/database/group/seeders/execute',
                    method: 'POST',
                    data: formData,
                })
                    .then((data) => {
                    ...
                    .catch((error) => {
                        ...
                        window.awesomeModal.alert(
                            'Error',
                            'Ha ocurrido un error al ejecutar el seeder del grupo ' +
                                group.name
                        );
                    });
            }
