...
const generateSeeder = (table) => {
    window.awesomeModal
        .confirm(
            'Generar seeder',
            '¿Está seguro de generar el seeder de la tabla ' + table + '?'
        )
        .then((value) => {
            if (value) {
                ...
                httpRequest({
                    url:
                        window.public_path +
                        '/api/database/table/seeders/generate',
                    method: 'POST',
                    data: formData,
                })
                    .then((data) => {
                    ...
                    .catch((error) => {
                        ...
                        window.awesomeModal.alert(
                            'Error',
                            'Ha ocurrido un error al generar el seeder de la tabla ' +
                                table
                        );
                    });
            }
...
const executeSeeder = (table) => {
    window.awesomeModal
        .confirm(
            'Ejecutar seeder',
            '¿Está seguro de ejecutar el seeder de la tabla ' + table + '?'
        )
        .then((value) => {
            if (value) {
                window.awesomeModal.loading();
                ...
                httpRequest({
                    url:
                        window.public_path +
                        '/api/database/table/seeders/execute',
                    method: 'POST',
                    data: formData,
                })
                    .then((data) => {
                    ...
                    .catch((error) => {
                        ...
                        window.awesomeModal.alert(
                            'Error',
                            'Ha ocurrido un error al ejecutar el seeder de la tabla ' +
                                table
                        );
                    });
            }
