<template>
    <div class="container modal">
        <div class="modal__header">
            <h1 class="controls__title">Foto: {{ data.rawData.photo.original_name }}</h1>
        ...
        <div class="modal__body grid-3 gap-15">
                ...
                <div class="col-1">
                    ...
                    <InputFake
                        label="Dimensiones"
                        placeholder=""
                        :text="data.rawData.photo.width + ' x ' + data.rawData.photo.height"
                        :error="[]"
                        :borderless="false"
                    />
                    <InputFake
                    ...
                    <div class="grid-2 gap-15 mt-15">
                        ...
                        <button class="btn btn btn--lightblue-outline col-1" @click="close">
                            <i class="fas fa-times"></i> Cerrar
                        </button>
                        <a class="btn btn btn--lightblue col-1" target="_blank" :href="data.rawData.photo.url">
    ...
    const deletePhoto = () => {
        window.awesomeModal.confirm(
            ...
            if (result) {
                ...
                httpRequest({
                    url: window.public_path + '/api/simple-photo-gallery/' + props.data.rawData.photo.id + '/delete',
                    method: 'GET'
                })
                .then((data) => {
                    modal.close()
                    window.awesomeModal.alert('¡Listo!, La foto se ha eliminado correctamente')
